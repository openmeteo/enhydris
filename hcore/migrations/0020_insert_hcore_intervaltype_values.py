...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        from enhydris.hcore.models import IntervalType
        keys = ['SUM', 'AVERAGE', 'MINIMUM', 'MAXIMUM',
                'VECTOR_AVERAGE',]
        #I use the key list to preserver the order
        values = {'SUM': "Sum", "AVERAGE": "Average value", 
                  "MINIMUM": "Minimum", "MAXIMUM": "Maximum",
                  "VECTOR_AVERAGE": "Vector average",}
        for value in keys:
            if IntervalType.objects.filter(value=value).exists():
                continue
            it = IntervalType(value=value, descr=values[value],
                                           descr_alt=values[value])
            it.save()
