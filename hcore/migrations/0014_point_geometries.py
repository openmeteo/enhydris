...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for gpoint in orm.Gpoint.objects.all():
            if gpoint.abscissa!=None and gpoint.ordinate!=None:
                if gpoint.srid==None: gpoint.srid = 4326
                gpoint.point = Point(x=gpoint.abscissa, y=gpoint.ordinate, 
                                     srid=gpoint.srid)
            gpoint.save()
    ...
    def backwards(self, orm):
        ...
        for gpoint in orm.Gpoint.objects.all():
            if gpoint.point!=None:
                if gpoint.srid==None: gpoint.srid = 4326
                gpoint.abscissa, gpoint.ordinate = gpoint.point.transform(gpoint.srid, clone=True)[0:2]
            gpoint.save()
