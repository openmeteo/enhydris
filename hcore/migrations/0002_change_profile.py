...
class Migration:
    ...
    def forwards(self, orm):
        ...
        # Changing field 'UserProfile.lname'
        # (to signature: django.db.models.fields.CharField(max_length=30, null=True, blank=True))
        db.alter_column('hcore_userprofile', 'lname', orm['hcore.userprofile:lname'])
        
        # Changing field 'UserProfile.fname'
        # (to signature: django.db.models.fields.CharField(max_length=30, null=True, blank=True))
        db.alter_column('hcore_userprofile', 'fname', orm['hcore.userprofile:fname'])
        
        # This is messy but using this we avoid the problem when migrating.
        # When we migrate all data once and store them in json format (we must
        # drop the sql file importing), this won't be an issue again
        try:
            db.delete_column('hcore_userprofile', 'is_public')
        except:
            print "Userprofile field \'is_public\' not found. Skipping..."
    ...
    def backwards(self, orm):
        ...
        db.add_column('hcore_userprofile', 'is_public',
                        orm['hcore.userprofile:is_public'])
