...
from enhydris.decorators import (gentityfile_permission,
...
def _station_csv(s):
    return [s.id, s.name, s.name_alt, s.short_name, s.short_name_alt,
            ...
            s.original_abscissa(), s.original_ordinate(),
            s.srid, s.approximate, s.altitude, s.asrid,
...
def _station_edit_or_create(request, station_id=None):
    ...
    if station_id:
        ...
        if not (user.has_row_perm(station, 'edit') and
                user.has_perm('enhydris.change_station')):
...
def station_delete(request, station_id):
    ...
    if not request.user.has_row_perm(station, 'delete') or not \
            request.user.has_perm('enhydris.delete_station'):
...
def timeseries_delete(request, timeseries_id):
    ...
    if not tseries or not related_station or not \
            ...
            request.user.has_perm('enhydris.delete_timeseries'):
        return HttpResponseForbidden('Forbidden',
                                     content_type='text/plain')
...
def gentityfile_delete(request, gentityfile_id):
    ...
    if not gfile or not related_station or \
            ...
            not request.user.has_perm('enhydris.delete_gentityfile'):
        return HttpResponseForbidden('Forbidden', content_type='text/plain')
...
def gentityevent_delete(request, gentityevent_id):
    ...
    if not gevent or not related_station or \
            not request.user.has_row_perm(related_station, 'edit') or \
            not request.user.has_perm('enhydris.delete_gentityevent'):
        return HttpResponseForbidden('Forbidden', content_type='text/plain')
...
def gentityaltcode_delete(request, gentityaltcode_id):
    ...
    if not galtcode or not related_station or \
            not request.user.has_row_perm(related_station, 'edit') or \
            not request.user.has_perm('enhydris.delete_gentityaltcode'):
        return HttpResponseForbidden('Forbidden', content_type='text/plain')
...
def overseer_delete(request, overseer_id):
    ...
    if not overseer or not related_station or \
            not request.user.has_row_perm(related_station, 'edit') or \
            not request.user.has_perm('enhydris.delete_overseer'):
        return HttpResponseForbidden('Forbidden', content_type='text/plain')
...
def instrument_delete(request, instrument_id):
    ...
    if not instrument or not related_station or \
            not request.user.has_row_perm(related_station, 'edit') or \
            not request.user.has_perm('enhydris.delete_instrument'):
        return HttpResponseForbidden('Forbidden', content_type='text/plain')
...
class ModelAddView(CreateView):
    ...
    def dispatch(self, *args, **kwargs):
        ...
        if not kwargs['model_name'] in ALLOWED_TO_EDIT or not \
                self.request.user.has_perm('enhydris.add_' +
                                           kwargs['model_name']):
            return HttpResponseForbidden('Forbidden',
                                         content_type='text/plain')
