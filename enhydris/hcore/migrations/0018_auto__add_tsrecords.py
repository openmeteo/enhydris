...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding table 'ts_records', but only if it does not already exist;
        # this is because in old versions ts_records was created outside
        # Django (see ticket #245).
        from django.db import connection, transaction
        cursor = connection.cursor()
        cursor.execute("SELECT COUNT(*) FROM pg_class WHERE relname='ts_records'")
        row = cursor.fetchone()
        if row[0]==1:
            return
        ...
        db.send_create_signal('hcore', ['TsRecords'])


    def backwards(self, orm):
        
        # And, by the way, we never delete ts_records
        return
