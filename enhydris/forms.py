...
from enhydris.widgets import SelectWithPop
...
class OverseerForm(ModelForm):
    ...
    station = forms.ModelChoiceField(station_objects, label="Station", empty_label=None)
    person = forms.ModelChoiceField(
        Person.objects, widget=SelectWithPop(model_name="person")
    )
...
class GpointForm(GentityForm):
    ...
    def clean(self):
        ...
        if bool("abscissa" in cleaned_data and cleaned_data["abscissa"]) ^ bool(
            "ordinate" in cleaned_data and cleaned_data["ordinate"]
        ):
            raise forms.ValidationError(
                _(
                    "Both coordinates (abscissa, "
                    "ordinate) should be "
        ...
        if ("abscissa" in cleaned_data and cleaned_data["abscissa"]) and not (
            ...
            self._errors["srid"] = self.error_class(
                [
                    "Since you have provided coordinates, "
...
def StationForm(*args, **kwargs):
    ...
    class StationFormClass(GpointForm, GentityForm):
        ...
        if settings.ENHYDRIS_USERS_CAN_ADD_CONTENT:
            maintainers = AutoCompleteSelectMultipleField("maintainers", required=False)

        def clean_altitude(self):
            value = self.cleaned_data["altitude"]
            if value is not None and (value > 8850 or value < -422):
                raise forms.ValidationError(_("%f is not a valid altitude") % (value,))
...
class InstrumentForm(ModelForm):
    ...
    station = forms.ModelChoiceField(
        station_objects, label="Stations", empty_label=None
    )
    type = forms.ModelChoiceField(
        InstrumentType.objects, widget=SelectWithPop(model_name="instrumenttype")
    )
...
class TimeseriesForm(ModelForm):
    ...
    gentity = forms.ModelChoiceField(
        Gentity.objects.all(), empty_label=None, label="Station"
    )
    instrument = forms.ModelChoiceField(
        Instrument.objects.all(), required=False, label="Instrument"
    )
    ...
    unit_of_measurement = forms.ModelChoiceField(
        UnitOfMeasurement.objects, widget=SelectWithPop(model_name="unitofmeasurement")
    )
    ...
    time_step = forms.ModelChoiceField(
        TimeStep.objects, widget=SelectWithPop(model_name="timestep"), required=False
    )
    ...
    def clean(self):
        ...
        timestamp_offset_months = self.cleaned_data.get("timestamp_offset_months", None)

        if (not time_step) and (
            timestamp_rounding_minutes
            or timestamp_rounding_months
            or timestamp_offset_minutes
            or timestamp_offset_months
        ):
            raise forms.ValidationError(
                _(
                    "Invalid Timestep: If time step is"
                    " null, the rounding and offset must also be null"
                )
            )
        elif time_step and (
            timestamp_offset_minutes is None or timestamp_offset_months is None
        ):
            raise forms.ValidationError(
                _(
                    "Invalid offset: If time step is"
                    " not null, timestamp offset values must be provided"
                )
            )
        elif time_step and (
            (
                timestamp_rounding_minutes is None
                and timestamp_rounding_months is not None
            )
            or (
        ...
        if instr:
            ...
            assert stat
            if Instrument.objects.filter(id=instr.id, station__id=stat.id).count() < 1:
                raise forms.ValidationError(
                    _("Selected instrument " "not in selected station")
                )
...
class TimeseriesDataForm(TimeseriesForm):
    ...
    data_policy = forms.ChoiceField(
        ...
        required=False,
        choices=(("A", "Append to existing"), ("O", "Overwrite existing")),
    )
