...
class GentityImageTestCase(APITestCase):
    def setUp(self):
        self.station = baker.make(models.Station)
        self.gentity_image = baker.make(
            models.GentityImage, gentity=self.station, descr="hello"
        )
...
@override_settings(ENHYDRIS_AUTHENTICATION_REQUIRED=False)
class GentityImageContentTestCase(APITestCase):
    def setUp(self):
        ...
        self.station = baker.make(models.Station)
        self.gentity_image = baker.make(models.GentityImage, gentity=self.station)
        with patch1, patch2:
...
@override_settings(ENHYDRIS_AUTHENTICATION_REQUIRED=False)
class GentityImageContentWithoutFileTestCase(APITestCase):
    def setUp(self):
        # Mommy creates a GentityImage without an associated file, so the
